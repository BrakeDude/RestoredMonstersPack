...
function mod:vesselCheckForLegOrWhateverTheFuckDSSSays(type, var, stype)
    if type == mod.ENTITY_INFO.VESSEL.ID then
        if RestoredMonsterPackJF.DSSavedata.vesselType == 2 then
    ...
    elseif vesselData.State == States.Attacking then
        ...
        if vesselSprite:IsEventTriggered("Shoot") then
            ...
            for i = 1, Settings.MaggotsToShoot do
                if Isaac.CountEntities(vessel,EntityType.ENTITY_SMALL_MAGGOT) < Settings.MaxMaggots then --vesselData.Maggots
...
function mod:vesselDamage(vessel, damageAmount, damageFlags, damageSource, damageCountdownFrames)
	...
	if Isaac.CountEntities(vessel,EntityType.ENTITY_SMALL_MAGGOT) >= Settings.MaxMaggots or vessel:GetDropRNG():RandomFloat() < Settings.MaggotCounterChance then return end
