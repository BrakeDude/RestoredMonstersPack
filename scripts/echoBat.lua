...
local function BlindBatScream()
	return RestoredMonsterPackJF:GetDSSData().blindBatScreamInc or 3
end
...
function mod:echoBatInit(entity)
	if entity.Variant == ECHO_BAT.VARIANT then
		...
		data.angleCountdown =
			mod:RandomIntBetween(rng, Settings.DirectionChangeTimes[1], Settings.DirectionChangeTimes[2])
...
function mod:echoBatUpdate(entity)
	if entity.Variant == ECHO_BAT.VARIANT then
		...
		if
			data.cooldown <= 0
			and target.Position:Distance(entity.Position) <= Settings.AttackRange
			and not sprite:IsPlaying("Attack")
			and not (
				entity:HasEntityFlags(EntityFlag.FLAG_FEAR)
				or entity:HasEntityFlags(EntityFlag.FLAG_SHRINK)
				or entity:HasEntityFlags(EntityFlag.FLAG_CONFUSION)
			)
		then
		...
		elseif sprite:IsEventTriggered("Shoot") then
			...
			entity:FireProjectiles(
				entity.Position,
				(target.Position - entity.Position):Normalized() * Settings.ShotSpeed,
				0,
				params
			)
		end
...
function mod:echoRingHit(target, damageAmount, damageFlags, damageSource, damageCountdownFrames)
	if
		damageSource.Type == EntityType.ENTITY_PROJECTILE
		and damageSource.Variant == ProjectileVariant.PROJECTILE_ECHO
	then
		if target.Type == EntityType.ENTITY_PLAYER then
			if
				not target:HasEntityFlags(EntityFlag.FLAG_CONFUSION)
				and not target:ToPlayer():HasCollectible(CollectibleType.COLLECTIBLE_EVIL_CHARM)
			then
			...
			target:AddConfusion(
				EntityRef(damageSource.Entity),
				90,
				0.8,
				Color(0.5, 0.5, 0.5, 1.0, 40 / 255, 40 / 255, 40 / 255)
			)
		end
...
function mod:confusionEffect(player, inputHook, buttonAction)
	if player and player:ToPlayer() and player:HasEntityFlags(EntityFlag.FLAG_CONFUSION) and isPolling == false then
		...
		if Input.IsActionPressed(buttonAction, player.ControllerIndex) then
			...
			data.MoveDirection = -player:GetMovementJoystick():Normalized()
				or player:GetRecentMovementVector():Normalized()
		else
...
function mod:confusionShader(shaderName)
	if shaderName == "ChromaticAbberation" then
		...
		distortion = Lerp(distortion, lerpTarget * (BlindBatScream() / 3 + 1), 0.1)
...
local function projectileKill(entity)
	for _, proj in pairs(Isaac.FindByType(EntityType.ENTITY_PROJECTILE, ProjectileVariant.PROJECTILE_TEAR)) do
		if proj.SpawnerEntity and GetPtrHash(proj.SpawnerEntity) == GetPtrHash(entity) then
			proj:Remove()
			FFGRACE:MakeSporeExplosion(proj.Position, proj.SpawnerEntity, 0.6)
		end
	end
end
...
function mod:chubbyBunnyUpdate(entity)
	if entity.Variant == mod.ENTITY_INFO.CHUBBY_BUNNY.VARIANT then
		...
		if (not sprite:IsPlaying("Death")) and (not sprite:IsPlaying("Transform")) then
			-- Movement
			data.vector = ((target.Position - entity.Position):Normalized() * Settings.ChaseSpeed):Rotated(
				data.angleOffset
			)
			if entity:HasEntityFlags(EntityFlag.FLAG_FEAR) or entity:HasEntityFlags(EntityFlag.FLAG_SHRINK) then
				data.vector = Vector(-data.vector.X, -data.vector.Y)
			...
			if
				not sprite:IsPlaying("Attack")
				and not sprite:IsPlaying("Idle")
				and not sprite:IsPlaying("Transform")
			then
				sprite:Play("Idle", true)
			end
			...
			elseif sprite:IsEventTriggered("Shoot") then
				entity:FireProjectiles(
					entity.Position,
					(target.Position - entity.Position):Normalized() * Settings.ShotSpeed * 1,
					0,
					params
				)
			...
			elseif sprite:IsEventTriggered("Shoot2") then
				entity:FireProjectiles(
					entity.Position,
					(target.Position - entity.Position):Normalized() * Settings.ShotSpeed * 1.2,
					0,
					params
				)
				entity:PlaySound(SoundEffect.SOUND_WHEEZY_COUGH, 1, 0, false, 1.2)
			elseif sprite:IsEventTriggered("Shoot3") then
				entity:FireProjectiles(
					entity.Position,
					(target.Position - entity.Position):Normalized() * Settings.ShotSpeed * 1.4,
					0,
					params
				)
				entity:PlaySound(SoundEffect.SOUND_WHEEZY_COUGH, 1, 0, false, 1.2)
...
function mod:chubbyBunnyProjectileUpdate(projectile)
	if
		FFGRACE
		and projectile.SpawnerEntity
		and projectile.SpawnerType == ECHO_BAT.ID
		and projectile.SpawnerVariant == mod.ENTITY_INFO.CHUBBY_BUNNY.VARIANT
	then
		FFGRACE:MakeSporeTrail(projectile, 0.75)
	end
end
...
function mod:chubbyBunnyProjectileCollision(projectile)
	...
	then
		projectile:Remove()
		FFGRACE:MakeSporeExplosion(projectile.Position, projectile.SpawnerEntity, 0.6)
	end
end
mod:AddCallback(
	ModCallbacks.MC_PRE_PROJECTILE_COLLISION,
	mod.chubbyBunnyProjectileCollision,
